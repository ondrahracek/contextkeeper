...
//   1. Environment variable: CK_STORAGE_PATH takes precedence
//   2. Explicit path: Appends ".contextkeeper" subdirectory to the provided path
//   3. Local context: Checks for .contextkeeper directory in current directory
//   4. Parent directories: Searches parent directories up to 10 levels for .contextkeeper
//   5. Global default: Falls back to OS-specific default location
//
// Parameters:
//   - explicitPath: A specific path to use; empty string triggers search strategy
//
// Returns:
//   - The resolved storage path as an absolute directory path to the .contextkeeper subdirectory
func (f *Finder) FindStoragePath(explicitPath string) string {
	...
	// 1. If explicit path provided, append .contextkeeper
	if explicitPath != "" {
